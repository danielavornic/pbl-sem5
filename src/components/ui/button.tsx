  ...
  "inline-flex items-center justify-center font-heading whitespace-nowrap rounded-lg font-semibold ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        ...
        outline: "border border-primary bg-card hover:bg-primary hover:text-primary-foreground",
      ...
      size: {
        ...
        xs: "h-6 rounded-md text-xs px-2",
      ...
      return React.Children.map(children as React.ReactElement, (child: React.ReactElement) => {
        return React.cloneElement(child, {
          ...
          children: (
            <>
              {loading && (
                <Loader2 className={cn("h-4 w-4 animate-spin", child.props.children && "mr-2")} />
              )}
